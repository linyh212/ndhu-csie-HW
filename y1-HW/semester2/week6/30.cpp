...
public:
	Rectangle(Segment *s): Shape(s, 4){
		if(seg[0].getLength()!=seg[2].getLength()||seg[1].getLength()!=seg[3].getLength()){
            throw invalid_argument("The given segments do not form a rectangle.");
        }
	}
	float getArea() override{
		float l=seg[0].getLength();
		float w=seg[1].getLength();
		return l*w;
	}
};

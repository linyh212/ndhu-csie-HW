...
sort_array:
    ...
    addi $a2, $t2, -1      # Set right index to size -1
    jal quick_sort         # Call quick_sort funct

    la $t0, array          # Load the base address of the array
    lw $t1, array_size     # Load the size of the array

print_loop:
    beqz $t1, exit         # If size == 0, exit the program
...
quick_sort:
    ...
    sw $a0, 4($sp)         # 	  array base address
    sw $a1, 8($sp)         # 	  left index
    sw $a2, 12($sp)        # 	  right
...
partition:
    ...
    add $t1, $a0, $t0      # Get the address of the rightmost number
